...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    ...
    if sandwich is None:
        raise HTTPException(status_code=404, detail="Sandwich is not found")
    return sandwich
...
@app.post("/order_details/", response_model=schemas.OrderDetail, tags=["Orders_details"])
...
@app.get("/order_details/", response_model=list[schemas.OrderDetail], tags=["Orders_details"])
def read_all_order_details(db: Session = Depends(get_db)):
    return order_details.read_all(db)

@app.get("/order_details/{order_detail_id}", response_model=schemas.OrderDetail, tags=["Orders_details"])
...
@app.put("/order_details/{order_detail_id}", response_model=schemas.OrderDetail, tags=["Orders_details"])
def update_one_order_detail(order_detail_id:int, order_detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    order_detail = order_details.update(db, order_detail_id=order_detail_id, order_detail=order_detail)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="Order is not found")
    return order_detail

@app.delete("/order_details/{order_detail_id}", tags=["Orders_details"])
